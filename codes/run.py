...
def parse_args(args=None):
    ...
    parser.add_argument('--test_batch_size', default=10,
...
def save_model(model, optimizer, save_variable_list, args, best_valid=False):
    ...
    '''
    if best_valid:
        logging.info('Saving best valid MRR model...')
        path = args.save_path + '/best'
    else:
...
def main(args):
    ...
    if args.init_checkpoint:
        ...
        checkpoint = torch.load(os.path.join(
            args.init_checkpoint, 'checkpoint'), map_location='cpu')
        ...
        for step in range(init_step, args.max_steps):
            ...
            if args.do_valid and step % args.valid_steps == 0 and step > 0:
                ...
                if metrics['MRR'] > best_valid_mrr:
                    save_model(kge_model, optimizer,
                               save_variable_list, args, True)
    ...
    if args.do_valid:
        ...
        metrics = kge_model.test_step(
            kge_model, valid_triples, all_true_triples, args)
        log_metrics('Valid', step, metrics)
        if args.do_train and metrics['MRR'] > best_valid_mrr:
            save_model(kge_model, optimizer,
                       save_variable_list, args, True)
            best_valid_mrr = metrics['MRR']
    ...
    if args.evaluate_train:
        ...
        metrics = kge_model.test_step(
            kge_model, train_triples, all_true_triples, args)
        log_metrics('Test', step, metrics)
